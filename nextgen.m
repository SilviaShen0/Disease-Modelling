% A comparison of the stochastic model vs the 'PMO Solver' model WITHOUT SUSCEPTIBILITY: 
...
S = transpose(sum(C_UK, 2)); % S_k values to calculate heuristic value of R0
...
T_2 = transpose(bsxfun(@times, C_UK, N_UK_prop));

dom_eig = max(abs(eig(T_2)));
...
T_3 = transpose(bsxfun(@times, C_UK, N_UK_prop));

dom_eig = max(abs(eig(T_3)));
