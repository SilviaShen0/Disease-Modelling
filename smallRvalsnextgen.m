% model WITHOUT SUSCEPTIBILITY: We investigate the PMOs for small values of R0, and plot the
% sub-population specific R0 values. beta is calculated using the NEXT-GEN matrix. 
...
S = transpose(sum(T_1, 2)); % S_k values to calculate heuristic value of R0
...
betaxx = beta*C_UK(1,1);
betaxy = beta*C_UK(1,2); 
betayx = beta*C_UK(2,1);
betayy = beta*C_UK(2,2);
